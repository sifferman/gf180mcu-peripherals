...
// + dpll_ctrl frequency-locked loop). Runs under Icarus with the FUNCTIONAL define so
...
//
//   iverilog -g2012 -DFUNCTIONAL -o /tmp/tb_dpll.vvp \
//       src/dpll/ring_dco.sv src/dpll/dpll_ctrl.sv cocotb/models/tb_dpll.v && vvp /tmp/tb_dpll.vvp
...
module tb_dpll;
  ...
  always @(posedge clk) begin
    if (lock) begin
      ...
      if (tune > 1 && tune < (1<<NUM_TUNE)-2)
        $display("PASS: dpll locked with tune=%0d in-range", tune);
      else
        $display("FAIL: dpll locked at a rail (tune=%0d)", tune);
      $finish;
    end
    cycles = cycles + 1;
  ...
  initial begin
    ...
    // dpll_ctrl actually see a reset edge, then release.
    rst_n  = 1'b1;
    enable = 1'b0;
    #2 rst_n = 1'b0;
