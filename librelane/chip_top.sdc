...
# -min 0.0 lets the SD data race the capture edge -> spurious input-port hold (~-7.5 ps at FF on
# bidir_PAD[2]). The SD card is source-synchronous and holds DAT/CMD well past the edge, so model the
# same ~10%-period input hold used for the RMII RX inputs above. Only affects hold (-max unchanged).
set_input_delay -clock $clocks -min $input_delay_min_value $sd_resp_in
