// verilog-ethernet UDP stack + udp_command_memory_bridge, presented to the board
// as an RMII PHY interface.
//
// Adapted from alexforencich's Arty UDP example: it wraps the native RMII MAC
// (verilog-ethernet eth_mac_rmii_fifo) plus a falling-edge TX retime so it drives
// the LAN8720 RMII pins directly, and the UDP echo loop is replaced by
// udp_command_memory_bridge.  Everything runs on clk_i; the MAC FIFO crosses to
// the 50 MHz RMII reference clock internally.
//
// Submodule (verilog-ethernet) and AXI port names are kept canonical so they
// match the upstream interface contract / Vivado AXI inference.
